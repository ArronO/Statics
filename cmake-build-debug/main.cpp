...
double thirdForce[] = {20, 21, 22, 23}; //magnitude, x, y, z
...
double *knownCO[3] = {firstForce, secondForce, thirdForce};
double *unknownCO[3] = {uFirstForce, uSecondForce, uThirdForce};

double *knownCOmag[3] = {firstForcemag, secondForcemag, thirdForcemag};
...
int main()
{
    ...
    for (int i = 1; i < 3; i++)
    {
        detz = detz + (unknownCOmag[i][0] * unknownCOmag[i+1][1] * unknownCOmag[i+2][2]);
        detz = detz - (unknownCOmag[i+2][0] * unknownCOmag[i+1][1] * unknownCOmag[i][2]);
    } //evaluates detz
